...
class Cleaner {
    async clean() {
        ...
        pipelines.forEach((p) => {
            const expirationTime = p.pipeline.startTime + (p.pipeline.options.ttl * 1000);
            if (expirationTime < Date.now()) {
                log.info(`pipeline ${p.jobId} started at ${this._formatDate(p.pipeline.startTime)} and expired at ${this._formatDate(expirationTime)}, sending stop to api-server.`, { component });
